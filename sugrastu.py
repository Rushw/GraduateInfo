# -*- coding: utf-8 -*-
import urllib, multiprocessing, datetime
import sys
...
class InfoExporter(object):
    ...
    def add_sheet(self, title):
        ...
        self.titles = [
            ...
            {"name": "家庭地区码", "width": 10,'id':'lbljtdqm'},
            {"name": "家庭地区", "width": 30,'id':'lbljtdq'},
            {"name": "入学方式", "width": 10,'id':'lblrxfs'},
            {"name": "入学前最后学历毕业院校码", "width": 10,'id':'lblbyyxm'},
            {"name": "最后毕业院校", "width": 10,'id':'lblbyyx'},
            {"name": "入学前最后学历毕业年月", "width": 20,'id':'lblbyrq'},
            {"name": "入学前工作单位", "width": 20,'id':'lblgzdw'},
            {"name": "是否留学人员", "width": 5,'id':'lblsflx'},
            {"name": "培养类别", "width": 10,'id':'lblpylb'},
            {"name": "专业", "width": 10,'id':'lblzymc'},
            {"name": "导师", "width": 10,'id':'lbldsxm'},
            {"name": "学位类型", "width": 10,'id':'lblxwlx'},
            {"name": "研究方向", "width": 10,'id':'lblyjfx'},
            {"name": "论文题目", "width": 100,'id':'lbllwtm'},
            {"name": "论文起止日期", "width": 60,'id':'lbllwqzrq'},
            {"name": "答辩日期", "width": 60,'id':'lbldbrq'},
            ...
            {"name": "分配单位类别", "width": 10,'id':'lblfpdwlb'},
            {"name": "学籍异动标记", "width": 10,'id':'lblxjyd'},
            {"name": "奖惩标记", "width": 10,'id':'lbljcbj'},
            {"name": "结束学业码", "width": 10,'id':'lbljsxy'},
            {"name": "结束学业年月", "width": 60,'id':'lbljsxyny'},
    ...
    def report(self):
        ...
        for num in range(0, 5000):
            ...
            for i,propty in enumerate(self.titles):
                sheet.cell(self._get_cell_string(i, rowNumber)
                           ).value = soup.find('span',{'id':propty['id']}).text
            rowNumber+=1
                
    def save(self):
        self._excel_writer.save(self.year + ".xlsx")
...
def main():
    ...
    if int(args[0]) < int(args[1]) and int(args[0]) > 0 and (2000 + int(args[1])) > datetime.datetime.now().year:
        ...
        sys.exit(1)
    procs = multiprocessing.Pool(processes=2 * multiprocessing.cpu_count())
    pargs = []
