...
class InfoExporter(object):
    ...
    def __init__(self, year):
        ...
        self._excel_writer = ExcelWriter(workbook=self._workbook,
                                         archive=ZipFile('%s.zip' % self.year, "a", ZIP_DEFLATED))
    ...
    def add_sheet(self, title):
        ...
        self.titles = [
            ...
            {"name": "授学位日期", "width": 30, 'id': 'lblsxwrq'},
    ...
    def request_data(self, num, sheet, rowNumber):
        ...
        if str(soup.find('span', {'id': 'lblxh'}).text).isdigit():
            ...
            self.nullcounter += 1
            if self.nullcounter > 20:
                return None
            return False
        return True
    def report(self):
        sheet = self._workbook.active
        initialRow = self.rowNumber = sheet.max_row + 1
        self.nullcounter = 0  # avoid continuing query the null ID section
        self.failnums = []
        for num in range(1, 6000):
            res = self.request_data(num, sheet, self.rowNumber)
            if res == True:
                self.rowNumber += 1
            elif res == False:
                continue
                ...
                break
        while len(self.failnums)>0:
            passnums = []
            for num in self.failnums:  # retry for timeout issue.
                res = self.request_data(num, sheet, self.rowNumber)
                if res == True:
                    passnums.append(num)
                    self.rowNumber += 1
                    ...
                    break
            self.failnums = [item for item in self.failnums if item not in passnums]
    def save(self):
